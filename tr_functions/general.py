...
class GeneralModel:
    ...
    def __init__(self):
        ...
        self._COLORSET = {1: '#1f77b4', 2: '#ff7f0e',
                          3: '#2ca02c', 4: '#d62728', 5: '#9467bd'}
    ...
    def print_model_result(self):
        ...
        print("\nConfusion matrix :")
        print("----------------")
        pd_cf = pd.DataFrame(self._conf_matrix, index=self._unique_classes,
                             columns=self._unique_classes, dtype=int)
    ...
    def plot_all_data(self):
        ...
        for class_nb in np.unique(np_train[:, 0].astype(np.int)):
            ...
            ax.scatter(x=np_train[idx_train, 1].astype(np.float), y=np_train[idx_train, 2].astype(
                np.float), c=self._COLORSET[class_nb], alpha=0.5, marker="+")
            ax.scatter(x=np_test[idx_test, 1].astype(np.float), y=np_test[idx_test, 2].astype(
                np.float), c=self._COLORSET[class_nb], label=class_nb, marker=".")
        ax.legend()
        plt.show()
    ...
    def print_conf_matrix(self):
        ...
        """
        df = pd.DataFrame(self._conf_matrix, index=self._unique_classes,
                          columns=self._unique_classes, dtype=int)
        print(df)
    ...
    def _add_train_point_to_plot(self):
        ...
        for class_nb in np.unique(np_train[:, 0].astype(np.int)):
            ...
            ax.scatter(x=np_train[idx, 1].astype(np.float), y=np_train[idx, 2].astype(
                np.float), c=self._COLORSET[class_nb], label=class_nb)
        ax.legend()
        ...
        for class_nb in np.unique(np_test[:, 0].astype(np.int)):
            ...
            for i in np_test[idx[0], 3]:
                if i != None:
                    ...
                    e_c.append("#000000")
            ax.scatter(x=np_test[idx, 1].astype(np.float), y=np_test[idx, 2].astype(
                np.float), edgecolors=self._COLORSET[class_nb], c=e_c, label=class_nb)
        ax.legend()
